...
> This repository uses **`agent.md`** at the workspace root as the top-level directive and **`.agents/`** as the customization root.
> Read this file first in every prompt and session. It is the single source of truth for rules, design decisions, skill capabilities, and reference paths.
