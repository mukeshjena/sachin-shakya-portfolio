...
import { ContactFields } from "./core-fields/ContactFields";
import { HeroFields } from "./core-fields/HeroFields";
import { MetricsFields } from "./core-fields/MetricsFields";
import { CapabilitiesFields } from "./fields/CapabilitiesFields";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[var(--ink-900)]/80 backdrop-blur-sm animate-fade-in">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="mt-5 space-y-5">
          ...
          {formData.type === "hero" && <HeroFields formData={formData} onChange={handleChange} />}

          {(formData.type === "impact" || formData.type === "telemetry") && (
            <MetricsFields formData={formData} onChange={handleChange} />
          )}
          ...
          {formData.type === "credentials" && (
            <CredentialsFields
              ...
              onChangeCertifications={(certs) => handleChange("certifications", certs)}
              onChangeEducation={(edu) => handleChange("education", edu)}
              onChangeAwards={(awards) => handleChange("awards", awards)}
            />
          )}

          {/* Narrative / Rich Markdown Editor (For Custom or supporting notes) */}
          {(formData.type === "custom" ||
            ...
            <div>
              <RichEditor
                label="Section Content & Narrative (Markdown / HTML / CSS)"
